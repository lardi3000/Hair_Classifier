...
void getGabor2(const Mat &src, vector<vector<float> > &local_energy, vector<vector<float> > &mean_amplitude, unsigned int kernel_size){
    ...
    float theta1[] = {0,45,90,135,158};
    vector<float> theta = vector<float>(theta1, theta1 + 5); //orientazione del filtro
    float psi1[] = {0, -45, -90};
    ...
    //A questo punto in output_gabor ho 72 mat con le risposte ai filtri.
